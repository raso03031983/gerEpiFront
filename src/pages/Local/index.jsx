...
import Actions from './divisao.action';
...
function Divisao() {
  const { ListDivisao = [], nwstate, divisao, error } = useSelector(state => state.Divisao);
  ...
  React.useEffect(() => {
    if (ListDivisao.length > 0) {
      const newListTable = ListDivisao.map((item) => { 
        const { id, descricao_Divisao } = item;
 
        return [ id, descricao_Divisao, 
  ...
  const handleSerch = () => {
    var param = {
      "descricao_Divisao" : formData.pesquisar,
      "id_cliente": 1
    }
    dispatch(Actions.getAll(param));
  }
  ...
  const Itens = id => {
    ...
    const data = ListDivisao.find(resp => resp.id === id);

    const _itens = {
      id: id,
      descricao_Divisao : data === undefined ? "" : data.descricao_Divisao,
    };
    return _itens;
  }
    ...
    {alert}
     <Card>
      <CardHeader color="info" icon>
          ...
          <h2 className={classes.cardIconTitle}>Divisão</h2>
        </CardHeader>
        <CardBody>
          <GridContainer>
