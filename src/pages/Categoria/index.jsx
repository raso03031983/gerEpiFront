...
function Categoria() {
  const { ListCategoria = [], nwstate, categoria, error } = useSelector(state => state.Categoria);
  ...
  React.useEffect(() => {
    if (ListCategoria.length > 0) {
      const newListTable = ListCategoria.map((item) => { 
        const { id, descricao_Categoria } = item;
 
        return [ id, descricao_Categoria,
  ...
  const handleSerch = () => {
    var param = {
      "descricao_Categoria" : formData.pesquisar,
      "id_cliente": 1
    }
    dispatch(Actions.getAll(param));
  }
  ...
  const Itens = id => {
    ...
    const data = ListCategoria.find(resp => resp.id === id);

    const _itens = {
      id: id,
      descricao_Categoria : data === undefined ? "" : data.descricao_Categoria,
    };
    return _itens;
  }
    ...
    {alert}
     <Card>
      <CardHeader color="info" icon>
          ...
          <h2 className={classes.cardIconTitle}>Categoria</h2>
        </CardHeader>
        <CardBody>
          <GridContainer>
